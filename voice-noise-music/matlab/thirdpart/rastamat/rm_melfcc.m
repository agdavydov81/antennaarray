function [cepstra,aspectrum,pspectrum] = rm_melfcc(samples, sr, varargin)
%[cepstra,aspectrum,pspectrum] = rm_melfcc(samples, sr[, opts ...])
...
%    'rm_lifterexp' (0.6): exponent for rm_liftering; 0 = none; < 0 = HTK sin rm_lifter
...
% (i.e. rm_melfcc(d,16000,opts...) =~= log(10)*2*mfcc(d*(2^17),16000) )
%       'wintime': 0.016
%     'rm_lifterexp': 0
%       'minfreq': 133.33
%       'maxfreq': 6855.6
...
% (i.e. rm_melfcc(d,16000,opts...) =~= 2*htkrm_melfcc(:,[13,[1:12]])'
%  where HTK config has PREEMCOEF = 0.97, NUMCHANS = 20, CEPrm_lifter = 22, 
...
%
% 2005-04-19 dpwe@ee.columbia.edu after rm_rastaplp.m.  
% Uses Mark Paskin's rm_process_options.m from KPMtools
...
pspectrum = rm_powspec(samples, sr, wintime, hoptime, dither);

aspectrum = rm_audspec(pspectrum, sr, nbands, fbtype, minfreq, maxfreq, sumpower, bwidth);

if (usecmp)
  % PLP-like weighting/compression
  aspectrum = rm_postaud(aspectrum, maxfreq, fbtype);
end
...
if modelorder > 0
  ...
  % LPC analysis 
  lpcas = rm_dolpc(aspectrum, modelorder);

  % convert lpc to cepstra
  cepstra = rm_lpc2cep(lpcas, numcep);

  % Return the auditory spectrum corresponding to the cepstra?
%  aspectrum = rm_lpc2spec(lpcas, nbands);
  % else return the aspectrum that the cepstra are based on, prior to PLP

else
  
  % Convert to cepstra via DCT
  cepstra = rm_spec2cep(aspectrum, numcep, dcttype);

end

cepstra = rm_lifter(cepstra, rm_lifterexp);
