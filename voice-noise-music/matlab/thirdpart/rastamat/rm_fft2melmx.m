function [wts,binfrqs] = rm_fft2melmx(nfft, sr, nfilts, width, minfrq, maxfrq, htkmel, constamp)
% wts = rm_fft2melmx(nfft, sr, nfilts, width, minfrq, maxfrq, htkmel, constamp)
...
%      Hence, Mel spectrum is rm_fft2melmx(nfft,sr)*abs(fft(xincols,nfft));
...
%      as rm_fft2melmx(512, 8000, 40, 1, 133.33, 6855.5, 0);
...
% 2004-09-05  dpwe@ee.columbia.edu  based on rm_fft2barkmx
...
minmel = rm_hz2mel(minfrq, htkmel);
maxmel = rm_hz2mel(maxfrq, htkmel);
binfrqs = rm_mel2hz(minmel+[0:(nfilts+1)]/(nfilts+1)*(maxmel-minmel), htkmel);

binbin = round(binfrqs/sr*(nfft-1));

for i = 1:nfilts
%  fs = rm_mel2hz(i + [-1 0 1], htkmel);
  fs = binfrqs(i+[0 1 2]);
  % scale by width
