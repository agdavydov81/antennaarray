...
%      Version: $Id: vb_lpcrand.m,v 1.4 2007/05/04 07:01:39 dmb Exp $
%
...
if p
   if ~bw
      ar=vb_lpcrf2ar(2*rand(n,p+1)-1);
   else
      k=exp(-pi*bw(:)*(0:p));
      if size(k,1)==1
         ar=vb_lpcrf2ar(2*rand(n,p+1)-1).*k(ones(n,1),:);
      else
         ar=vb_lpcrf2ar(2*rand(n,p+1)-1).*k;
      end
   end
else
   ar=ones(n,1);
