function [y,fs,hd,ffx]=vb_readsfs(ff,ty,sub,mode,nmax,nskip,xpath)
%vb_readsfs  Read a .SFS format sound file [Y,FS,HD,FFX]=(FF,TY,SUB,MODE,NMAX,NSKIP,XPATH)
%
...
%                 can be the ffx output from a previous call to vb_readsfs
...
%             'd'    Look in data directory: vb_voicebox('dir_data')
...
%      Version: $Id: vb_readsfs.m,v 1.4 2007/05/04 07:01:39 dmb Exp $
%
...
if nargin<7
    xpath=vb_voicebox('sfsbin');       % path for sfs programs
end
EXESUF=vb_voicebox('sfssuffix');                                              % suffix for executable O/S dependent
if nargin<4 mode='p';
else mode = [mode(:).' 'p'];
end
if nargout==0
    if nargin<2 | ty<=0
        [yy,fs,hd,ffx]=vb_readsfs(ff,0,0,mode);
        fprintf(1,'File: %s\n',ffx{1});
        fprintf(1,'Database: %s, Speaker: %s, Date: %s, Token: %s\n',hd{6},hd{7},hd{9},hd{10});
        lst=ffx{3};
        for it=2:size(lst,1);
            [yy,fs,hd,ffx]=vb_readsfs(ffx,lst(it,1),lst(it,2),mode,0);
            nf=hd{1}(7);
            fd=hd{1}(9);
            fprintf(1,'%3d.%02d %ss @ %sHz (%d frames): %s\n',lst(it,1),lst(it,2),vb_sprintsi(nf*fd),vb_sprintsi(1/fd),nf,ffx{4}{it,1});   
        end
    end
else
    it=[];
    ...
    while (isempty(it))                 % may go round this loop twice
        if ischar(ff)           % If ff is a string we must read file
            if any(mode=='d')
                ff=fullfile(vb_voicebox('dir_data'),ff);
            end
            fid=fopen(ff,'rb','b');
            ...
            for i=2:200
                ...
                proglist{i,1}=char(vb_zerotrim(t(1:256)'));
                proglist{i,2}=char(vb_zerotrim(t(257:384)'));
                proglist{i,3}=char(vb_zerotrim(t(437:456)'));
    ...
    if ~lit(1)                              % read main header
        ...
        if nargout>2
            ...
            for i=1:13
                hd{i+1}=char(vb_zerotrim(mb(mc(i,1):mc(i,2))));
            end
        end
    else
