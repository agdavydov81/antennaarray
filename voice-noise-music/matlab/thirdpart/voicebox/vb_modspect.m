function [c,qq,ff,tt,po]=vb_modspect(s,fs,m,nf,nq,p)
%vb_modspect Calculate the modulation spectrum of a signal C=(S,FS,W,NC,P,N,INC,FL,FH)
%
%
% Simple use:
%
...
%
%           r/n/m   time domain vb_windows: rectangular/hanning/hamming [def: hamming]
%           T/N/M   mel/mod-mel domain vb_windows: triangular/hanning/hamming [def: triangular]
%
...
%      Version: $Id: vb_modspect.m,v 1.10 2009/09/21 10:08:48 dmb Exp $
%
...
fx=vb_rfft(vb_enframe(s,w,ni),nf,2);
...
[mbk,ffm]=vb_melbankm(po.fbin,nf,fs,po.fmin/fs,min(po.fmax/fs+(po.fmax==0),0.5),po.fwin);
...
mx=vb_rfft(reshape(px(ix(:),:),mnw,nmt*npf),mnf,1);          % find modulation spectrum
% nmm=size(mx,1);             % number of modulation spectrum bins [not needed]
[qbk,qqm]=vb_melbankm(po.mbin,mnf,100*fs/ni,po.mmin*ni/fs,min(po.mmax*ni/fs+(po.mmax==0),0.5),po.mwin);    % multiply frq by 100 to make it alsmost log
nqq=length(qqm);
switch po.mpow
    case 'a'
...
if po.qdcq
    dx=reshape(vb_rdct(reshape(qx,nqq,nmt*npf)),nqq,nmt,npf);    % take dct in q direction
...
if po.qdcp
    dx=permute(reshape(vb_rdct(reshape(permute(qx,[3,1,2]),npf,nqq*nmt)),npf,nqq,nmt),[2 3 1]);    % take dct in p direction
...
if bitand(po.dplt,512)
    ...
    ffx=repmat(vb_mel2frq(ffm(:)),1,nqq)/1000;
    qqx=repmat(vb_mel2frq(qqm(:)')/100,npf,1);
    plot(qqx,ffx,'xb');
