function h=vb_entropy(p,dim,cond,arg,step)
%vb_entropy calculates the vb_entropy of discrete and sampled continuous distributions H=(P,DIM,COND,ARG,STEP)
%
%  Inputs:  P        is a vector or matrix of probabilities - one dimension per variable
%           DIM      lists dimensions along which to evaluate the vb_entropy [default: 1st non singleton dimension]
...
%
% Outputs:  H        is the vb_entropy. It will have the same number of dimensions as the length of the ARG input.
%                    If the STEP argument is specified then this will be the differential vb_entropy.
%
% Example: Suppose P(W,X,Y,Z) represents the joint probability of four correlated random variables
%
%               (a) H(W,X,Y,Z) = vb_entropy(P,[1 2 3 4]). 
%               (b) H(W) = vb_entropy(P), or equivalently vb_entropy(P,1)
%               (c) H(W,Z | X,Y) = vb_entropy(P,[1 4],[2 3])
%               (d) H(W | X, Z=z) = vb_entropy(P,1,2,4); this is a function of z and will be a column vector
%
...
%      Version: $Id: vb_entropy.m,v 1.3 2007/05/04 07:01:38 dmb Exp $
%
...
if ~length(cond)
    ...
    h=vb_entropy(p,[dim(:); cond(:)],0,arg)-vb_entropy(p,cond,0,arg);
end
