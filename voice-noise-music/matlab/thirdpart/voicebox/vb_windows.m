function w = vb_windows(wtype,n,mode,p)
%vb_windows Generate a standard windowing function (TYPE,N,MODE,P)
%
...
%                                the window [default for most vb_windows].
%                  n [neither] = The first and last samples are one sample away from the ends
%                                of the window [default for vb_windows having zero end points].
...
%     is given in the table below. For overlapping vb_windows with perfect reconstruction choose
...
% the window vb_windows('hamming',n,'sa2q') where the 2 is the overlap factor
% (i.e. it should be replaced by 4 for 75% overlap).

%      Copyright (C) Mike Brookes 2002-2005
%      Version: $Id: vb_windows.m,v 1.8 2009/07/08 15:23:12 dmb Exp $
%
...
kk=[-1 1 1 -1; 0 0 2 -2; 0 1 2 -1;   % mode  w,  h,  c  [normal vb_windows]
...
switch wtype
    ...
    case 'dolph'
        ...
        np=1;               % number of parameters = 1
        if rem(ks+kp,2)     % for shifted vb_windows, we generate twice as many points
