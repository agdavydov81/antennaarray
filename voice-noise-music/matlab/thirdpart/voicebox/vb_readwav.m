function [y,fs,wmode,fidx]=vb_readwav(filename,mode,nmax,nskip)
%vb_readwav  Read a .WAV format sound file [Y,FS,WMODE,FIDX]=(FILENAME,MODE,NMAX,NSKIP)
%
% Input Parameters:
%
%	FILENAME gives the name of the file (with optional .WAV extension) or alternatively
%                 can be the FIDX output from a previous call to vb_readwav
...
%	WMODE    mode string needed for vb_writewav to recreate the data file
...
%      Version: $Id: vb_readwav.m,v 1.6 2007/05/04 07:01:39 dmb Exp $
%
...
if nargin<1 error('Usage: [y,fs,wmode,fidx]=vb_readwav(filename,mode,nmax,nskip)'); end
if nargin<2 mode='p';
else mode = [mode(:).' 'p'];
end
