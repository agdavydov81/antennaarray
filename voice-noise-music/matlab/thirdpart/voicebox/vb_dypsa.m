function [gci,goi] = vb_dypsa(s,fs)
%vb_dypsa   Derive glottal closure instances from speech [gci,goi] = (s,fs)
...
%        Instants in Voiced Speech using the vb_dypsa Algorithm,” IEEE Trans on Speech and Audio
...
%   [3]  A. Kounoudes, P. A. Naylor, and M. Brookes, “The vb_dypsa algorithm for estimation of glottal
...
%       The following parameters are defined in vb_voicebox()
%
...
%         6. Allow vb_dypsa to be called in chunks for a long speech file
...
%        12. Remove qmin>2 condition from voicespurt start detection (vb_dypsa 2 compatibility) in two places
...
%      Version: $Id: vb_dypsa.m,v 3.6 2007/05/04 07:01:38 dmb Exp $
%
...
dy_preemph=vb_voicebox('dy_preemph');
dy_lpcstep=vb_voicebox('dy_lpcstep');
dy_lpcdur=vb_voicebox('dy_lpcdur');
dy_dopsp=vb_voicebox('dy_dopsp');              % Use phase slope projection (1) or not (0)?
dy_ewtaper=vb_voicebox('dy_ewtaper');        % Prediction order of FrobNorm method  in seconds
dy_ewlen=vb_voicebox('dy_ewlen');        % windowlength of FrobNorm method  in seconds
dy_ewdly=vb_voicebox('dy_ewdly');        % shift for assymetric speech shape at start of voiced cycle
dy_cpfrac=vb_voicebox('dy_cpfrac');        % presumed ratio of larynx cycle that is closed
dy_lpcnf=vb_voicebox('dy_lpcnf');          % lpc poles per Hz (1/Hz)
dy_lpcn=vb_voicebox('dy_lpcn');            % lpc additional poles
...
[ar, e, k] = vb_lpcauto(s_used,lpcord,floor([dy_lpcstep dy_lpcdur]*fs));
...
dy_gwlen=vb_voicebox('dy_gwlen');          % group delay evaluation window length
dy_fwlen=vb_voicebox('dy_fwlen');          % window length used to smooth group delay
...
[tew,sew]=vb_zerocros(y,'n');              % find zero crossings
...
dy_fxminf=vb_voicebox('dy_fxminf');
frob=frob(:)';
mm=round(fs/dy_fxminf);
mfrob=vb_maxfilt(frob,1,mm);
...
% get algorithm parameters from vb_voicebox()

dy_fxmin=vb_voicebox('dy_fxmin');        % min larynx frequency (Hz)
dy_fxmax=vb_voicebox('dy_fxmax');        % min larynx frequency (Hz)
dy_xwlen=vb_voicebox('dy_xwlen');        % cross-correlation length for waveform similarity (sec)
dy_nbest=vb_voicebox('dy_nbest');        % Number of NBest paths to keep
dy_spitch=vb_voicebox('dy_spitch');              % scale factor for pitch deviation cost
wproj=vb_voicebox('dy_cproj');           % cost of projected candidate
dy_cspurt=vb_voicebox('dy_cspurt');           % cost of a talkspurt
dy_wpitch=vb_voicebox('dy_wpitch');           % DP pitch weighting
dy_wener=vb_voicebox('dy_wener');           % DP energy weighting
dy_wslope=vb_voicebox('dy_wslope');           % DP group delay slope weighting
dy_wxcorr=vb_voicebox('dy_wxcorr');           % DP cross correlation weighting
...
% rather complicated window specification is for compatibility with vb_dypsa 2
...
%     end
    ...
    if qmin<=qmax
        ...
        % === fudge to match vb_dypsa2.4 - could more efficiently be added
...
%         f_costs(rix,6)=reshape(q_cas(1+floor((nbix(1:dy_nbest)-1)/dy_nbest)),dy_nbest,1);
        ...
        % ==== the qmin>2 condition is for compatibility with vb_dypsa 2 and
        ...
        if (qmin>2) && (f_c(f_fb(qmin-1))+wproj*(1-gcic(r,2))<f_c(iNb))        % compare with worst of Nbest paths
            f_f(iNb)=f_fb(qmin-1);
            % === for now we exclude the energy and phase-slope costs for compatibility with vb_dypsa2
...
% === for compatibility with vb_dypsa2, we force the penultimate candidate to be accepted
