function [fx,tt]=vb_fxrapt(s,fs,mode,f0_rg)
%vb_fxrapt RAPT pitch tracker [FX,VUV]=(S,FS,MODE,F0_RG)
%
% Input:   s(ns)      Speech signal
%          fs         Sample frequency (Hz)
%          mode       'g' will plot a graph [default if no output arguments]
%          f0_rg      pitch frequency range
%
...
for iframe=1:nframe       % loop for each frame (~10 ms)
    ...
    [ipkd,vpkd]=vb_findpeaks(nccfd,'q');

    % Debugging: execute the line below to plot the autocorrelation peaks.
    % vb_findpeaks(nccfd,'q'); xlabel(sprintf('Lag = (x+%d)*%g ms',minlag-1,1000*kdsmp/fs)); ylabel('Normalized Cross Correlation'); title (sprintf('Frame %d/%d',iframe,nframe));
    ...
    if iframe>1                         % if it is not the first frame, then calculate pitch consistency and v/uv transition costs
        ...
        if mc+mcp
			mnk_s=zeros(size(rmswin));	% Manik's patch
			mnk_rg=fho+[rmsix(1) rmsix(end)]-kdrms;
			mnk_rg=max(1, mnk_rg);
			mnk_s(end-(mnk_rg(2)-mnk_rg(1)):end)=s(mnk_rg(1):mnk_rg(2));
			mnk_sp=zeros(size(rmswin));
			mnk_sp(end-(mnk_rg(2)-mnk_rg(1)):end)=sp(mnk_rg(1):mnk_rg(2));

			rr=sqrt((rmswin'*s(fho+rmsix).^2)/(rmswin'*mnk_s.^2)); % amplitude "gradient"
            ss=0.2/(vb_distitar(vb_lpcauto(sp(fho+rmsix),lpcord),vb_lpcauto(mnk_sp,lpcord),'e')-0.8);   % Spectral stationarity: note: Talkin uses Hanning instead of Hamming vb_windows for LPC
...
nf=pow2(nextpow2(ny));
w=vb_irfft(repmat(conj(vb_rfft(x,nf,1)),1,my).*vb_rfft(y,nf,1));
s=zeros(ny+1,my);
