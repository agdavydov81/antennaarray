function db=vb_lpccc2db(cc,np,nc)
%vb_lpccc2dbf Convert complex cepstrum to dB power spectrum DB=(CC,NP,NC)
...
%      Version: $Id: vb_lpccc2db.m,v 1.4 2007/05/04 07:01:38 dmb Exp $
%
...
k=10/log(10);
if nc==mc
   db=k*vb_rsfft([zeros(nf,1) cc].',2*np+2).';
else
   db=k*vb_rsfft([zeros(nf,1) vb_lpccc2cc(cc,nc)].',2*np+2).';
end
if nargout==0
