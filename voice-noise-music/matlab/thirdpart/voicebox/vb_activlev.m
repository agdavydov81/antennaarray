function [lev,af,fso,fsx]=vb_activlev(sp,fs,mode)
%vb_activlev Measure active speech level as in ITU-T P.56 [LEV,AF,FSO]=(SP,FS,MODE)
%
...
%
%            fso=fs; for i=1:inc:nsamp, [lev,fso]=vb_activlev(sp(i:i+inc-1),fso,mode); end
%
%            is equivalent to:                lev=vb_activlev(sp(1:nsamp),fs,mode)
%
%            but is much slower. The two methods will not give identical results
%            because they will use slightly different threshods.

%For completeness we list here the vb_contents of the FSO structure:
%
...
%      Version: $Id: vb_activlev.m,v 1.8 2009/09/13 15:49:35 dmb Exp $
%
...
if ~isstruct(fs)                        % no state vector given
    ...
    fso.zx=-Inf;                        % initial value for vb_maxfilt()
...
if ns                       % process this speech chunk
    ...
    [qe,qk,fso.zx]=vb_maxfilt(qe,1,fso.nh,1,fso.zx);       % apply the 0.2 second hangover
    oemax=fso.emax;
