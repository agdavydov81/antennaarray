function wts = rm_fft2barkmx(nfft, sr, nfilts, width, minfreq, maxfreq)
% wts = rm_fft2barkmx(nfft, sr, nfilts, width)
...
%      Hence, Bark spectrum is rm_fft2barkmx(nfft,sr)*abs(fft(xincols,nfft));
% 2004-09-05  dpwe@ee.columbia.edu  based on rastamat/rm_audspec.m
...
min_bark = rm_hz2bark(minfreq);
nyqbark = rm_hz2bark(maxfreq) - min_bark;
...
binbarks = rm_hz2bark([0:nfft/2]*sr/nfft);
...
function z = rm_hz2bark(f)
%       rm_hz2bark         Converts frequencies Hertz (Hz) to Bark
% taken from rastamat
z = 6 * asinh(f/600);
