function [x,eql] = rm_invpostaud(y,fmax,fbtype,broaden)
%x = rm_postaud(y,fmax,fbtype)
%
% invert the effects of rm_postaud (loudness equalization and cube
% root compression)
% y = rm_postaud output
...
if strcmp(fbtype, 'bark')
  bandcfhz = rm_bark2hz(linspace(0, rm_hz2bark(fmax), nbands));
elseif strcmp(fbtype, 'mel')
  bandcfhz = rm_mel2hz(linspace(0, rm_hz2mel(fmax), nbands));
elseif strcmp(fbtype, 'htkmel') || strcmp(fbtype, 'fcmel')
  bandcfhz = rm_mel2hz(linspace(0, rm_hz2mel(fmax,1), nbands),1);
else
