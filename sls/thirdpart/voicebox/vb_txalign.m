function [kx,ky,nxy,mxy,sxy]=vb_txalign(x,y,maxt,nsd)
%vb_txalign Find the best alignment of two sets of time markers [KX,KY,N,M,S]=(X,Y,MAXT)
...
% e.g. vb_txalign([1 11 21 27 31 42 51],[2 12 15 22 25 32 41 52 61],1.1);
...
%
% If no outputs are specified, vb_txalign plots a graph showing y-x against mean(x,y)

%      Copyright (C) Mike Brookes 2002
%      Version: $Id: vb_txalign.m,v 1.4 2007/05/04 07:01:39 dmb Exp $
%
...
if ~nargout
    ...
    [kx,ky,nxy,mxy,sxy]=vb_txalign(x,y,maxt);
    nxy0=nxy+1;
    while nxy<nxy0
        nxy0=nxy;
        mxy0=mxy;
        [kx,ky,nxy,mxy,sxy]=vb_txalign(x+mxy0,y,nsd*sxy);
    end
    mxy=mxy+mxy0;
    if ~nargout
        vb_txalign(x,y,nsd*sxy); % plot it
    end
end
else
    kx=zeros(lx,1);
