function [y,fs,ffx]=vb_readsph(filename,mode,nmax,nskip)
%vb_readsph  Read a SPHERE/TIMIT format sound file [Y,FS,FFX]=(FILENAME,MODE,NMAX,NSKIP)
%
% Input Parameters:
%
%	FILENAME gives the name of the file (with optional .SPH extension) or alternatively
%                 can be the FFX output from a previous call to vb_readsph having the 'f' mode option
...
%      Version: $Id: vb_readsph.m,v 1.7 2009/09/13 15:50:57 dmb Exp $
%
...
if nargin<1 error('Usage: [y,fs,hdr,fidx]=vb_readsph(filename,mode,nmax,nskip)'); end
if nargin<2 mode='p';
else mode = [mode(:).' 'p'];
end
    ...
    if ksamples>0
        ...
        if icode>=10 & ~length(ffx{5})
            ...
            if floor(icode/10)==1               % shorten
                doscom=['"' vb_voicebox('shorten') '"' ' -x -a ' num2str(info(3)) ' "' filename '" "' filetemp '"'];
