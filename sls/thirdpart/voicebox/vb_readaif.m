function [y,fs,wmode,fidx]=vb_readaif(filename,mode,nmax,nskip)
%vb_readaif  Read a .AIF format sound file [Y,FS,WMODE,FIDX]=(FILENAME,MODE,NMAX,NSKIP)
%
% Input Parameters:
%
%	FILENAME gives the name of the file (with optional .AIF extension) or alternatively
%                 can be the FIDX output from a previous call to vb_readaif
...
%      Version: $Id: vb_readaif.m,v 1.5 2007/05/04 07:01:39 dmb Exp $
%
...
if nargin<1 error('Usage: [y,fs,wmode,fidx]=vb_readaifF(filename,mode,nmax,nskip)'); end
...
if ~info(3)
    ...
    if filen~=0
        fprintf(2,'vb_readaif warning: Inconsistent file length - %d extra bytes\n',filen);
    end
end
fs=info(9);
