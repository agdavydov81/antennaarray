function [x,zo,xs]=vb_estnoisem(yf,tz,pp)
%vb_estnoisem - estimate noise spectrum using minimum statistics
% Inputs:
...
%
% Example use:      y=vb_enframe(s,w,ni);                  % divide speech signal s(n) into
%                                                       % overlapping frames using window w(n)
%                   yf=vb_rfft(y,nf,2);                    % take fourier transform
%                   dp=vb_estnoisem(yf.*conj(yf),tinc);    % estimate the noise
%
% If convenient, you can call vb_estnoisem in chunks of arbitrary size. Thus the following are equivalent:
%
%                   (a) dp=vb_estnoisem(yp(1:300),tinc);
%
%                   (b) [dp(1:100),z]=vb_estnoisem(yp(1:100),tinc);
%                       [dp(101:200),z]=vb_estnoisem(yp(101:200),z);
%                       [dp(201:300),z]=vb_estnoisem(yp(201:300),z);
...
%      Version: $Id: vb_estnoisem.m,v 1.1 2008/05/22 17:17:02 dmb Exp $
%
...
if isempty(yf) && isstruct(tz)             % no real data
    ...
    if isstruct(tz)       % take parameters from a previous call
        ...
        qq.nu=8;           % number of subvb_windows
    ...
    nu=qq.nu;           % number of subvb_windows
    ...
    if isempty(yf)      % provide dummy initialization
        ...
        for t=1:nr              % we use t instead of lambda in the paper
            ...
            bminv=1+2*(nv-1)*(1-mv)./(qeqi.^(-1)-2*mv);      % same expression but for sub vb_windows
