function [m,v,w,g,f,pp,gg]=vb_gaussmix(x,c,l,m0,v0,w0)
%vb_gaussmix fits a gaussian mixture pdf to a set of data observations [m,v,w,g,f]=(x,c,l,m0,v0,w0)
%
...
%                'k'    k-means algorithm ('kf' and 'kp' determine initialization of vb_kmeans)
%                'h'    k-harmonic means algorithm ('hf' and 'hp' determine initialization of vb_kmeans)
...
%      Version: $Id: vb_gaussmix.m,v 1.22 2009/09/21 14:03:13 dmb Exp $
%
...
memsize=vb_voicebox('memsize');    % set memory size to use
if isempty(c)
    c=1/n^2;
else
...
if nargin<6             % no initial values specified for m0, v0, w0
    ...
    if n<=k                         % each data point can have its own mixture
        ...
        if any(v0=='k')                         % k-means initialization
            if any(v0=='m')
                [m,e,j]=vb_kmeans(xs,k,m);
            elseif any(v0=='p')
                [m,e,j]=vb_kmeans(xs,k,'p');
            else
                [m,e,j]=vb_kmeans(xs,k,'f');
            end
        elseif any(v0=='h')                     % k-harmonic means initialization
            if any(v0=='m')
                [m,e,j]=vb_kmeanhar(xs,k,[],4,m);
            else
                if any(v0=='p')
                    [m,e,j]=vb_kmeanhar(xs,k,[],4,'p');
                else
                    [m,e,j]=vb_kmeanhar(xs,k,[],4,'f');
                end
            end
        elseif any(v0=='p')                     % Initialize using a random partition
            j=ceil(rand(n,1)*k);                % allocate to random clusters
            j(vb_rnsubset(k,n))=1:k;               % but force at least one point per cluster
            for i=1:k
                m(i,:)=mean(xs(j==i,:),1);
            end
        else
            if any(v0=='m')
                ...
                m=xs(vb_rnsubset(k,n),:);          % Forgy initialization: sample k centres without replacement [default]
            end
            [e,j]=vb_kmeans(xs,k,m,0);             % find out the cluster allocation
        end
        if any(v0=='s')
