function d=vb_distitar(ar1,ar2,mode)
%vb_distitar calculates the Itakura distance between AR coefficients D=(AR1,AR2,MODE)
%
...
%
% The Itakura distance is gain-independent, i.e. vb_distitpf(f*pf1,g*pf2) is independent of f and g.
%
% The Itakura distance may be expressed as log(ar2*toeplitz(vb_lpcar2rr(ar1))*ar2') where the ar1 and ar2 polynomials
...
%
%         np=255; d0=vb_distitar(ar1,ar2); d1=vb_distitpf(vb_lpcar2pf(ar1,np),vb_lpcar2pf(ar2,np))
%
...
%      Version: $Id: vb_distitar.m,v 1.5 2007/05/04 07:01:38 dmb Exp $
%
...
if any(mode=='d') | (mode~='x' & nf1==nf2)
   nx=min(nf1,nf2);
   d=2*sum(vb_lpcar2rr(ar1(1:nx,:)).*m2(1:nx,:),2).*((ar1(1:nx,1)./ar2(1:nx,1)).^2);
else
   d=2*vb_lpcar2rr(ar1)*m2'.*((ar1(:,1)*ar2(:,1)'.^(-1)).^2);
end
if all(mode~='e')
    d=log(d);
end