function [x,esq,j] = vb_kmeanlbg(d,k)
%vb_kmeanlbg Vector quantisation using the Linde-Buzo-Gray algorithm [X,ESQ,J]=(D,K)
%
%Inputs:
...
%      Version: $Id: vb_kmeanlbg.m,v 1.4 2007/05/04 07:01:38 dmb Exp $
%
...
nc=size(d,2);
[x,esq,j]=vb_kmeans(d,1);
m=1;
while m<k
   n=min(m,k-m);
   m=m+n;
   e=1e-4*sqrt(esq)*rand(1,nc);
   [x,esq,j]=vb_kmeans(d,m,[x(1:n,:)+e(ones(n,1),:); x(1:n,:)-e(ones(n,1),:); x(n+1:m-n,:)]);
end
