function c=vb_melcepst(s,fs,w,nc,p,n,inc,fl,fh)
%vb_melcepst Calculate the mel cepstrum of a signal C=(S,FS,W,NC,P,N,INC,FL,FH)
%
%
% Simple use: c=vb_melcepst(s,fs)	% calculate mel cepstrum with 12 coefs, 256 sample frames
%				  c=vb_melcepst(s,fs,'e0dD') % include log energy, 0th cepstral coef, delta and delta-delta coefs
%
% Inputs:
%     s	 speech signal
...
%       (2) get vb_rdct to change the data length (properly) instead of doing it explicitly (wrongly)

%      Copyright (C) Mike Brookes 1997
%      Version: $Id: vb_melcepst.m,v 1.7 2009/10/19 10:20:32 dmb Exp $
%
...
if any(w=='R')
   z=vb_enframe(s,n,inc);
elseif any (w=='N')
   z=vb_enframe(s,hanning(n),inc);
else
   z=vb_enframe(s,hamming(n),inc);
end
f=vb_rfft(z.');
[m,a,b]=vb_melbankm(p,n,fs,fl,fh,w);
pw=f(a:b,:).*conj(f(a:b,:));
pth=max(pw(:))*1E-20;
if any(w=='p')
   y=log(max(m*pw,pth));
