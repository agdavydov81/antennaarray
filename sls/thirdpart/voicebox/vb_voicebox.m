...
%                    when the "d" option is specified in vb_readwav etc.
%    PP.shorten      location of SHORTEN executable. SHORTEN is a proprietary file compression
%                    algorithm that is used for some SPHERE-format files. vb_readsph
...
%                    is given to vb_readsfs, it will try to create a requested item
%                    if it is not present in the SFS file. This parameter tells it
%                    where to find the SFS executables.
%    PP.sfssuffix    suffix for Speech Filing Sysytem binaries. vb_readsfs uses this paremeter
...
if isempty(PP)
    ...
    % vb_dypsa glottal closure identifier
    ...
    if exist(PP.dir_temp)~=7        % check that temp directory exists
        PP.dir_temp = vb_winenvar('temp');     % else use vb_windows temp directory
    end
