function d=vb_distisar(ar1,ar2,mode)
%vb_distisar calculates the Itakura-Saito distance between AR coefficients D=(AR1,AR2,MODE)
%
...
% A symmetrical version is the COSH distance: vb_distchpf(x,y)=(vb_distispf(x,y)+vb_distispf(y,x))/2
%
% The I-S distance can be expressed as ar2*toeplitz(vb_lpcar2rr(ar1))*ar2' + log((ar1(1)/ar2(1)).^2) - 1
...
%
%         np=255; d0=vb_distisar(ar1,ar2); d1=vb_distispf(vb_lpcar2pf(ar1,np),vb_lpcar2pf(ar2,np))
%
% Autocorrelation LPC analysis is equivalent to minimizing the Itakura-Saito difference between the
% signal spectrum and that of the all-pole LPC filter, i.e. vb_distispf(pf,vb_lpcar2pf(ar0,np)).
...
%
%               vb_distispf(pf,vb_lpcar2pf(ar,np)) = vb_distispf(pf,vb_lpcar2pf(ar0,np)) + vb_distisar(ar0,ar)
...
%      Version: $Id: vb_distisar.m,v 1.4 2007/05/04 07:01:38 dmb Exp $
%
...
if any(mode=='d') | (mode~='x' & nf1==nf2)
   nx=min(nf1,nf2);
   d=2*sum(vb_lpcar2rr(ar1(1:nx,:)).*m2(1:nx,:),2)-log((ar2(1:nx,1)./ar1(1:nx,1)).^2)-1;;
else
   d=2*vb_lpcar2rr(ar1)*m2'-log((ar1(:,1).^(-1)*ar2(:,1)').^2)-1;
end
