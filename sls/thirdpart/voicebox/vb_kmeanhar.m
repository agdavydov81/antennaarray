function [x,g,xn,gg] = vb_kmeanhar(d,k,l,e,x0)
%vb_kmeans Vector quantisation using K-harmonic means algorithm [X,G,XN,GG]=(D,K,L,E,X0)
%
%  Inputs:
%
...
% dimension before calling vb_kmeanhar so that approximately equal weight is given
...
%      Version: $Id: vb_kmeanhar.m,v 1.6 2008/06/02 07:20:52 dmb Exp $
%
...
if ischar(x0)
    if k<n
        if any(x0=='p')                  % Initialize using a random partition
            ...
            ix(vb_rnsubset(k,n))=1:k;      % but force at least one point per cluster
            x=zeros(k,p);
            for i=1:k
                x(i,:)=mean(d(ix==i,:),1);
            end
        else                                % Forgy initialization: choose k random points [default]
            x=d(vb_rnsubset(k,n),:);         % sample k centres without replacement
        end
    else
