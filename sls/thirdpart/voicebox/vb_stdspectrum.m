function [b,a]=vb_stdspectrum(s,m,f,n,zi)
%vb_stdspectrum Generate standard acoustic/speech spectra in s- or z-domain [B,A]=(S,M,F,N,ZI)
%
...
%   9  vb_usasi      : Noise simulating long-term programme material spectrum from [7],[8].
...
% Other candidates: (a) Z-weighting, (b) ISO226, (c) vb_usasi, (d) POTS
%
% Other standards:
...
%      Version: $Id: vb_stdspectrum.m,v 1.4 2009/06/03 11:59:01 dmb Exp $
%
...
if isempty(spz)
    spty={'White';'A-Weight';'B-Weight';'C-Weight';'LTASS-P50';'LTASS-1994';'SII-IntInv';'BS-468';'vb_usasi';'POTS'};
    spz={[1 0];
...
switch m1
    ...
    case 't'
        if nargin<5
            [b,a]=vb_randfilt(bz,az,n);
        else
            [b,a]=vb_randfilt(bz,az,n,zi);
        end
    case 'm'
        b = abs(h);
    case 'f'
