function [ss,zo]=vb_ssubmmse(si,fsz,pp)
%vb_ssubmmse performs speech enhancement using mmse estimate of spectral amplitude or log amplitude [SS,ZO]=(S,FSZ,P)
%
% Inputs:
...
%        pp.nu        % number of subvb_windows to use [3]
...
%
%
% If convenient, you can call vb_specsub in chunks of arbitrary size. Thus the following are equivalent:
%
%                   (a) y=vb_ssubmmse(s,fs);
%
%                   (b) [y1,z]=vb_ssubmmse(s(1:1000),fs);
%                       [y2,z]=vb_ssubmmse(s(1001:2000),z);
%                       y3=vb_ssubmmse(s(2001:end),z);
%                       y=[y1; y2; y3];
%
...
% with the output from the next call to vb_specsub().
%
% Refs:
%    [1] Ephraim, Y. & Malah, D.
...
%      Version: $Id: vb_ssubmmse.m,v 1.1 2008/05/27 20:36:28 dmb Exp $
%
...
if isstruct(fsz)
    ...
    if nargin>=3 && ~isempty(pp)
        qp=pp;      % save for vb_estnoisem call
        qqn=fieldnames(qq);
        for i=1:length(qqn)
            if isfield(pp,qqn{i})
                qq.(qqn{i})=pp.(qqn{i});
...
if isstruct(fsz)
    [dp,ze]=vb_estnoisem(yp,ze);   % estimate the noise using minimum statistics
    ssv=fsz.ssv;
    xu=fsz.xu;                  % saved unsmoothed SNR
else
    [dp,ze]=vb_estnoisem(yp,tinc,qp);   % estimate the noise using minimum statistics
...
if ~nr                                  % no data frames
    ...
    se=(vb_irfft((yf.*g).',nf).').*repmat(w,nr,1);   % inverse dft and apply output window
...
if nargout>1
    ...
    zo.qp=qp;                       % save vb_estnoisem parameters
