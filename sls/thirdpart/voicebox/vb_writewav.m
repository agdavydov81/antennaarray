function fidx=vb_writewav(d,fs,filename,mode,nskip)
%vb_writewav Creates .WAV format sound files FIDX=(D,FS,FILENAME,MODE,NSKIP)
%
%   The input arguments for vb_writewav are as follows:
%
...
%                        alternatively the FIDX output from a previous vb_writewav call
...
%
%   Note: vb_writewav will create an 16-bit PCM, auto-scaled wave file by default.
...
%      Version: $Id: vb_writewav.m,v 1.4 2007/05/04 07:01:39 dmb Exp $
%
...
if nargin<3 error('Usage: vb_writewav(data,fs,filename,mode,nskip)'); end
...
if nc>10 error('vb_writewav: attempt to write a sound file with >10 channels'); end
nc=max(nc,1);
...
if ischar(filename)
    if any(mode=='d')
        filename=fullfile(vb_voicebox('dir_data'),filename);
    end
    ny=nyd;
